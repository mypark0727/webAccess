...
$(function () {
  ...
  $lp_open.on("click", function () {
    const op = $(this); // 클릭한 요소에 대한 객체
    const lp = $("#" + op.attr("aria-controls")); // 연결된 모달에 대한 객체
    const lpObj = lp.children(".popup_inner"); // 모달 내부의 콘텐츠에 대한 객체
    const lpBody = lpObj.find(".popup_body"); // 모달 스크롤 콘텐츠 영역에 대한 객체
    const isBottomSheet = lp.hasClass("bottom_sheet"); //모달 바텀시트 일때
    ...
    if (isBottomSheet) {
      ...
      lpObjHandle.on("mousedown touchstart", (e) => {
        ...
        $(document).on("mousemove touchmove", (e) => {
          ...
          // syncObj 높이 한계값 설정
          let currentHeight = lpObj.outerHeight();
          let limitHeight = currentHeight >= $(window).height() * 0.75 ? $(window).height() * 0.75 : `${currentHeight}px`;
          syncObj.animate({
            bottom: limitHeight
          }, 0);
